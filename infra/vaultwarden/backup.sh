...
# key file lives on the host. Retention: the 10 newest db_*.sqlite3 objects.
set -euo pipefail
...
# 3. Prune to the newest 10 (ls prints full gs:// URIs, one per line).
gcloud storage ls "${BUCKET}/" | LC_ALL=C sort | head -n -10 | while read -r old; do
  [ -n "${old}" ] || continue
